<?php
// tools/png-to-webp-converter.php
session_start();
...
$pageTitle = ($currentLang === 'tr') ? 'PNG to WebP Dönüştürücü' : 'PNG to WebP Converter';
$pageDescription = ($currentLang === 'tr') ? 
    'PNG görsellerini WebP formatına ücretsiz dönüştürün. %90 daha küçük dosya boyutu.' :
    'Convert PNG images to WebP format for free. 90% smaller file size.';

function convertPngToWebp($sourceFile, $targetFile, $quality = 80) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['png_file'])) {
    ...
    if ($uploadedFile['error'] !== UPLOAD_ERR_OK) {
        ...
        if ($detectedType !== 'image/png') {
            $error = ($currentLang === 'tr') ? 'Lütfen sadece PNG dosyası yükleyin!' : 'Please upload only PNG files!';
            ...
            if (!function_exists('imagewebp')) {
                ...
                if (move_uploaded_file($uploadedFile['tmp_name'], $sourcePath)) {
                    if (convertPngToWebp($sourcePath, $targetPath, $quality)) {
                        $result = [
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h5><i class="fas fa-info-circle"></i> 
                                <?php echo ($currentLang === 'tr') ? 'PNG to WebP' : 'PNG to WebP'; ?>
                            </h5>
                            <p><?php echo ($currentLang === 'tr') ? 
                                'PNG dosyalarınızı modern WebP formatına dönüştürerek %90\'a kadar küçültün.' :
                                'Convert PNG files to modern WebP format and reduce size by up to 90%.'; ?>
                            </p>
                            <div class="alert alert-success">
                                <small><i class="fas fa-leaf"></i> <?php echo ($currentLang === 'tr') ? 
                                    'WebP format çevre dostu - daha az bant genişliği kullanır!' :
                                    'WebP format is eco-friendly - uses less bandwidth!'; ?></small>
                            </div>
                        </div>
                    </div>
                    <?php endif; ?>
